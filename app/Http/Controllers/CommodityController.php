...
class CommodityController extends Controller
{
    ...
    public function index()
    {
        ...
        $query->when(request()->filled('nomor_seri'), function ($q) {
            return $q->where('nomor_seri', 'like', '%' . request('nomor_seri') . '%');
        });

        $query->when(request()->filled('ukuran'), function ($q) {
            return $q->where('ukuran', 'like', '%' . request('ukuran') . '%');
        });

        $query->when(request()->filled('warna'), function ($q) {
